        ...
        <div class="max-w-4xl mx-auto px-6">
            ...
            <section class="mb-10">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="text-3xl font-light tracking-wide" style="color: #4f4842;">帮助中心</h1>
                </div>
            ...
            <div class="mb-8">
                <div class="relative">
                    <input type="text" v-model="searchQuery"
                        ...
                        style="background: rgba(255, 250, 245, 0.6); border: 1px solid #e7dbd0; color: #4b423c; font-weight: 300;"
                        placeholder="搜索帮助内容..." @focus="e => e.currentTarget.style.borderColor = '#dccfc4'"
                        @blur="e => e.currentTarget.style.borderColor = '#e7dbd0'">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm" style="color: #b8aa98;">🔍</span>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2 mb-8">
                <button v-for="cat in categories" :key="cat.value" @click="activeCategory = cat.value"
                    class="px-4 py-1.5 text-xs font-light transition-all rounded-full" :style="activeCategory === cat.value
            ...
            <!-- ====== 帮助列表 ====== -->
            <div class="space-y-3">
                <div v-for="item in filteredHelpItems" :key="item.id"
                    ...
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-3">
                                ...
                                <h3 class="text-base font-light" style="color: #4f4842;">{{ item.question }}</h3>
                            </div>
                            <div v-show="expandedId === item.id" class="mt-3 pt-3"
                                style="border-top: 1px solid rgba(231, 219, 208, 0.3);">
                                <p class="text-sm font-light leading-relaxed"
                                    style="color: #5f5547; white-space: pre-line;">{{ item.answer }}</p>
                            </div>
                        </div>
                        <span class="text-sm transition-transform" style="color: #b8aa98;"
                            :style="{ transform: expandedId === item.id ? 'rotate(180deg)' : 'rotate(0deg)' }">
                            ▼
                        </span>
                    </div>
                </div>
            ...
            <!-- 空状态 -->
            <div v-if="filteredHelpItems.length === 0" class="text-center py-12"
                style="background: rgba(255, 250, 245, 0.3); border-radius: 3rem; border: 1px dashed #e7dbd0;">
                <div class="text-4xl mb-3">🔍</div>
                <p class="text-sm font-light" style="color: #8a7e74;">没有找到相关帮助内容</p>
                <p class="text-xs font-light mt-1" style="color: #b8aa98;">试试其他关键词</p>
            </div>

            <!-- ====== 联系支持 ====== -->
            <div class="mt-8 p-6 rounded-[2rem] text-center"
                ...
                <p class="text-xs font-light mt-1" style="color: #8a7e74;">联系我们，我们会尽快回复你</p>
                <a href="mailto:admin@cldery.com"
...
const helpItems = ref([
    {
        ...
        icon: '👤',
        question: '如何注册账号？',
        answer: '点击页面右上角的「开始体验」或「登录」按钮，使用统一账号（Casdoor）授权登录即可。首次登录会自动创建账号。',
        category: 'account'
    },
    {
        id: '2',
        icon: '👤',
        question: '如何修改个人信息？',
        answer: '目前个人信息（如用户名、头像）由统一账号系统同步管理。你可以在登录页通过统一账号系统修改这些信息，修改后会自动同步到栖所。',
        category: 'account'
    },
    {
        id: '3',
        icon: '👤',
        question: '如何注销账号？',
        answer: '如需注销账号，请发送邮件至 admin@cldery.com 联系我们。我们会核实身份后协助你完成注销，并删除所有相关数据。',
        category: 'account'
    },
    {
        id: '4',
        icon: '📝',
        question: '如何记录心情？',
        answer: '登录后进入「仪表盘」，点击「记录心情」按钮。选择对应的情绪图标（如😊开心、😌平静等），可以调整强度，添加文字备注和标签。',
        category: 'mood'
    },
    {
        id: '5',
        icon: '📝',
        question: '心情记录可以删除吗？',
        answer: '可以。在「仪表盘」的近期心情列表中，找到对应的记录，点击删除按钮即可移除。该操作不可撤销，请谨慎操作。',
        category: 'mood'
    },
    {
        id: '6',
        icon: '📝',
        question: '什么是情绪标签？',
        answer: '情绪标签可以帮助你更细致地记录情绪触发因素。你可以选择预设标签（如「工作」「学习」「家庭」等），也可以自定义标签，让记录更加个性化。',
        category: 'mood'
    },
    {
        id: '7',
        icon: '💬',
        question: '如何分享帖子？',
        answer: '进入「社区」页面，点击「分享你的方法」按钮。可以选择「便签」或「帖子」两种形式，填写内容后发布即可。',
        category: 'community'
    },
    {
        id: '8',
        ...
        answer: '便签是简短的小方法或暖心提醒（类似便利贴），适合快速分享一个小技巧。帖子是更详细的经验分享，可以包含更完整的故事和方法。',
        category: 'community'
    },
    {
        id: '9',
        icon: '💬',
        question: '如何删除自己的内容？',
        answer: '在「社区」页面，找到你发布的帖子或便签。每个内容卡片下方都有「删除」按钮（仅对你自己的内容可见），点击即可删除。',
        category: 'community'
    },
    {
        id: '10',
        icon: '🔒',
        question: '我的数据安全吗？',
        answer: '栖所非常重视你的数据安全。所有数据通过 HTTPS 加密传输，密码经过哈希加密存储。我们不会出售或分享你的个人数据给第三方。',
        category: 'privacy'
    },
    {
        id: '11',
        ...
        answer: '你的心情记录是私有的，仅你自己可见。只有你在社区主动分享的帖子或便签是公开可见的。',
        category: 'privacy'
    },
    {
        id: '12',
        icon: '🔒',
        question: '平台如何保护用户隐私？',
        answer: '我们遵循最小必要原则收集信息，仅收集提供核心功能所需的基本数据。所有数据加密存储，并严格按照隐私政策处理。详细内容请查看「隐私政策」页面。',
        category: 'privacy'
    },
])
