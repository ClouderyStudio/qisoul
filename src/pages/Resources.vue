      ...
      <div class="space-y-10">
        ...
        <section v-if="activeCategory === 'all' || activeCategory === '网站'">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div
              v-for="item in filteredWebsites"
              :key="item.id"
        ...
        <section v-if="activeCategory === 'all' || activeCategory === '练习'">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div
              v-for="item in filteredPractices"
              :key="item.id"
              class="p-6 transition-all rounded-[2rem]"
              style="
                background: var(--c-card-bg);
...
const apps = ref([
  {
    ...
    title: "脑电波",
    description:
      "专注于治愈与陪伴的跨平台情绪记录软件。100% 本地存储，不上传云端，给你的内心世界最安全的物理保护。包含情绪记录、日记、日历、统计分析、心理测评、用药提醒、隐私锁等功能。",
    platform: "Flutter",
...
const filteredWebsites = computed(() => {
  if (activeCategory.value === "all" || activeCategory.value === "网站")
    return websites.value;
  return [];
});

const filteredApps = computed(() => {
  if (activeCategory.value === "all" || activeCategory.value === "APP")
    return apps.value;
  return [];
});

const filteredPractices = computed(() => {
  if (activeCategory.value === "all" || activeCategory.value === "练习")
